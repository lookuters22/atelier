-- Playbook rule candidate review: atomic approve / reject / supersede + promotion to playbook_rules.
-- Approve-time optional overrides (candidate row unchanged). Explicit global vs channel upsert (no naive ON CONFLICT on NULL channel).

ALTER TABLE public.playbook_rule_candidates
  ADD COLUMN IF NOT EXISTS reviewed_at TIMESTAMPTZ;

ALTER TABLE public.playbook_rule_candidates
  ADD COLUMN IF NOT EXISTS reviewed_by_photographer_id UUID NULL
    REFERENCES public.photographers(id) ON DELETE SET NULL;

COMMENT ON COLUMN public.playbook_rule_candidates.reviewed_at IS
  'When a terminal review action (approve/reject/supersede) was applied.';
COMMENT ON COLUMN public.playbook_rule_candidates.reviewed_by_photographer_id IS
  'Photographer who performed the terminal review action.';
