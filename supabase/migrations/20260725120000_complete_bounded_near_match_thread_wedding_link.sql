-- Bounded near-match escalation approval: link thread to suggested wedding + finalize escalation + clear V3 hold (atomic).
-- Service-role only; mirrors link_thread_to_wedding routing metadata append semantics without auth.uid().

ALTER TABLE public.escalation_resolution_jobs
  ADD COLUMN IF NOT EXISTS approve_bounded_near_match_thread_link boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.escalation_resolution_jobs.approve_bounded_near_match_thread_link IS
  'When true, worker runs complete_bounded_near_match_thread_wedding_link instead of generic learning-loop resolution.';
...
BEGIN
  ...
  -- Same semantics as link_thread_to_wedding (append manual_link_history + manual_link snapshot)
...
COMMENT ON FUNCTION public.complete_bounded_near_match_thread_wedding_link(uuid, uuid, text) IS
  'Approves bounded_matchmaker_near_match: links thread only when threads.wedding_id IS NULL (CAS); returns thread_already_linked if already linked; clears V3 hold when hold escalation id matches; finalizes escalation (thread_wedding_link).';
