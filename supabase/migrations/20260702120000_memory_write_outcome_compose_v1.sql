-- Memory write-path v1: optional explicit outcome + deterministic summary composition (Ana + escalation paths).

CREATE OR REPLACE FUNCTION public.compose_memory_summary_with_outcome(
  p_outcome text,
  p_summary text,
  p_max int
) RETURNS text
...
AS $$
  SELECT CASE
    WHEN p_outcome IS NOT NULL AND btrim(p_outcome) <> '' THEN
      CASE
        WHEN p_summary IS NOT NULL AND btrim(p_summary) <> '' AND btrim(p_summary) IS DISTINCT FROM btrim(p_outcome) THEN
          left(btrim(p_outcome) || ' — ' || btrim(p_summary), p_max)
        ELSE
          left(btrim(p_outcome), p_max)
      END
    ELSE
      left(coalesce(p_summary, ''), p_max)
  END;
$$;

-- Replace 7-arg overload with 8-arg (trailing p_outcome optional for SQL callers).
DROP FUNCTION IF EXISTS public.complete_escalation_resolution_memory(
  uuid,
  uuid,
  uuid,
  text,
  text,
  text,
  public.escalation_learning_outcome
);
...
BEGIN
  ...
  IF v_mem_id IS NULL THEN
    INSERT INTO public.memories (
      ...
      left(p_title, 120),
      public.compose_memory_summary_with_outcome(
        nullif(trim(both from p_outcome), ''),
        p_summary,
        400
      ),
...
REVOKE ALL ON FUNCTION public.complete_escalation_resolution_memory(
  uuid, uuid, uuid, text, text, text, public.escalation_learning_outcome, text
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.complete_escalation_resolution_memory(
  uuid, uuid, uuid, text, text, text, public.escalation_learning_outcome, text
) TO service_role;

-- Learning-loop memory artifacts: optional JSON `outcome` composes into summary; omitted => prior behavior.
  ...
  LOOP
    ...
    ELSIF v_kind = 'memory' THEN
      ...
      INSERT INTO public.memories (
        ...
        public.compose_memory_summary_with_outcome(
          nullif(trim(both from v_elem->>'outcome'), ''),
          COALESCE(v_elem->>'summary', ''),
          400
        ),
