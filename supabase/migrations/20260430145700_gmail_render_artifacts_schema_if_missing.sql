-- Repair: duplicate 20260412120000 versions may have applied phase10 instead of G3 on remote.
-- Idempotent: ensures gmail_render_artifacts + messages FK exist before later RPC migrations.

CREATE TABLE IF NOT EXISTS public.gmail_render_artifacts (
...
CREATE INDEX IF NOT EXISTS idx_gmail_render_artifacts_photographer
  ON public.gmail_render_artifacts (photographer_id);
CREATE INDEX IF NOT EXISTS idx_gmail_render_artifacts_import_candidate
  ON public.gmail_render_artifacts (import_candidate_id)
  WHERE import_candidate_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_gmail_render_artifacts_message
...
DROP POLICY IF EXISTS "gmail_render_artifacts_tenant_isolation" ON public.gmail_render_artifacts;
