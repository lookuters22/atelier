...
export type InvoiceSetupProposalReviewAction = "reject" | "withdraw";

/**
 * Move a `pending_review` row to `rejected` or `withdrawn` via
 * `review_invoice_setup_change_proposal` (SECURITY DEFINER RPC — no direct table UPDATE from client).
 */
export async function reviewInvoiceSetupChangeProposal(
  supabase: SupabaseClient<Database>,
  params: { proposalId: string; action: InvoiceSetupProposalReviewAction },
): Promise<{ ok: true } | { ok: false; error: string }> {
  const p_next_status = (params.action === "reject" ? "rejected" : "withdrawn") as const;

  const { error } = await supabase.rpc("review_invoice_setup_change_proposal", {
